...
function registerEvn(argvs) {
  if (argvs.notify_sckey) {
    // Server酱 SCKEY
    process.env[''] = argvs.notify_sckey
  }
  
  if (argvs.notify_sctkey) {
    // Server酱 SCKEY
    process.env[''] = argvs.notify_sctkey
  }

  if (argvs.notify_dingtalk_token) {
    // dingtalk access_token
    process.env[''] = argvs.notify_dingtalk_token
  }
  ...
  if (argvs.notify_tele_bottoken && argvs.notify_tele_chatid) {
    // 机器人TOKEN
    process.env[''] = argvs.notify_tele_bottoken
    // CHAT ID
    process.env[''] = argvs.notify_tele_chatid
  }
