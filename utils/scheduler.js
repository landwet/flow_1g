...
let scheduler = {
 ...
 initTasksQueue: async () => {
    ...
    genFileName(command) {
        ...
        console.info('🚨获得高速通道🚨', maskFile, '当前日期', scheduler.today)
    },
    loadTasksQueue: async (selectedTasks) => {
       ...
       console.info('️⚠️⚠️⚠️⚠️⚠️⚠️北风提示北风提示,只有游戏流量以及游戏积分!全程5分钟,请手动营业厅领取⚠️⚠️⚠️⚠️⚠️⚠️',queues.length,queues.filter(t => t.taskState === 1).length,queues.filter(t => t.taskState === 2).length, selectedTasks.length, will_tasks.length)
	     return {
            taskJson,
            queues,
            will_tasks
    ...
    hasWillTask: async (command, params) => {
        ...
        if (scheduler.isTryRun) {
            console.info('⚠️温馨提示您已进入高速通道☢安全驾驶⚠️')
            await new Promise((resolve) => setTimeout(resolve, 3000))
        }
        process.env['taskKey'] = [command, scheduler.taskKey].join('_')
        process.env['command'] = command
        console.info('🎴获得通证识别码🃏', scheduler.taskKey.replaceWithMask(2, 3), '☑️')
    ...
    execTask: async (command) => {
        console.info('🚀已驶入高速通道,自动驾驶5分钟完成☢')
